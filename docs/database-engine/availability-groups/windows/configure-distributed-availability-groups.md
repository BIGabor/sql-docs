...
1. Test the failover readiness. Run the following query on both node, replace distributedag with your Distributed AlwaysOn Group name :
    ...
    ```  
    The availability group is ready to fail over when the **synchronization_state_desc** is `SYNCHRONIZED` and the **last_redone_lsn** per AG database is the same on both nodes. 
...
After completing the steps above, the distributed availability group fails over without any data loss. If the availability groups are across a geographical distance that causes latency, change the availability mode back to ASYNCHRONOUS_COMMIT by running the code from point 1. on *both* the global primary and the forwarder, but this time with AVAILABILITY_MODE = ASYNCHRONOUS_COMMIT. 
